...
import { Upload, Loader2, Image as ImageIcon, X, Check, ChevronDown } from 'lucide-react'
...
export function AdvertiseForm({ activeStates, activeLgas }: { activeStates: string[], activeLgas: Record<string, string[]> }) {
  ...
  const selectAllLgas = (state: string) => {
    const stateLgas = activeLgas[state] || []
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const targetStates = activeStates.filter(state => 
      (activeLgas[state] || []).some(lga => selectedLgas.includes(lga))
    )
    ...
    try {
      ...
      if (res.ok) {
        ...
        const dashboardPath = user.role === 'candidate' ? '/dashboard/candidate' 
              ...
              <div className="space-y-1 text-center">
                {imageUrl ? (
                  <div className="relative inline-block">
                    <Image src={imageUrl} alt="Preview" width={300} height={200} className="max-h-40 mx-auto rounded shadow-sm" />
          ...
          <div>
            ...
            <div className="space-y-2">
              {activeStates.map(state => {
                const stateLgas = activeLgas[state] || []
                const isAllSelected = stateLgas.length > 0 && stateLgas.every(lga => selectedLgas.includes(lga))
