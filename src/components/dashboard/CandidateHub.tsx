...
export function CandidateHub({ candidateId, initialName, candidateLga, avatarUrl }: { candidateId: string, initialName: string, candidateLga: string, avatarUrl?: string }) {
    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-10">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="bg-white dark:bg-[#11241b] border border-border dark:border-[#1f3a2c] rounded-2xl p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-serif text-lg font-bold text-ink dark:text-white">Campaign Promises</h3>
              <button onClick={() => setIsAddingPromise(!isAddingPromise)} className="text-forest hover:bg-forest-faint dark:hover:bg-[#1b3a2b] p-1 rounded">
            ...
            {isAddingPromise && (
              <div className="mb-4 flex flex-col gap-2 animate-slide-down">
                <Textarea 
                  ...
                  rows={2}
                  className="text-sm bg-white dark:bg-[#0f1d16] dark:text-white dark:border-[#1f3a2c]"
                />
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
              {promises.length === 0 && <p className="text-sm text-muted dark:text-[#c0d0c4]">No promises added yet.</p>}
              {promises.map(p => (
                <div key={p.id} className="border-l-4 border-forest dark:border-forest-700 pl-3 py-1">
                  <p className="text-sm font-medium text-ink dark:text-white">{p.text}</p>
                  <div className="flex gap-2 mt-1">
                    <button 
                      onClick={() => updatePromiseStatus(p.id, 'fulfilled')} 
                      className={`text-[10px] px-2 py-0.5 rounded ${p.status === 'fulfilled' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' : 'bg-gray-100 dark:bg-[#0f1d16] text-gray-500 dark:text-[#c0d0c4]'}`}
                    >
                      Fulfilled
                    </button>
                    <button 
                      onClick={() => updatePromiseStatus(p.id, 'in-progress')} 
                      className={`text-[10px] px-2 py-0.5 rounded ${p.status === 'in-progress' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300' : 'bg-gray-100 dark:bg-[#0f1d16] text-gray-500 dark:text-[#c0d0c4]'}`}
                    >
                      In Progress
                    </button>
                  </div>
        ...
        <div className="md:col-span-2 space-y-6">
          {/* Composer */}
          <div className="bg-white dark:bg-[#11241b] border border-border dark:border-[#1f3a2c] rounded-2xl p-6">
            <h3 className="font-serif text-lg font-bold text-ink dark:text-white mb-4">Post an Update</h3>
            <Textarea 
              value={postText} 
            ...
            <div className="flex justify-between items-center mt-4 pt-4 border-t border-border-light dark:border-[#1f3a2c]">
              <div className="flex gap-2">
                <label className="cursor-pointer p-2 rounded-lg hover:bg-forest-faint dark:hover:bg-[#1b3a2b] text-muted dark:text-[#c0d0c4]">
          ...
          <div className="space-y-4">
            {posts.length === 0 && <p className="text-center text-muted dark:text-[#c0d0c4] py-8">No posts yet. Share your first update!</p>}
            {posts.map(post => (
